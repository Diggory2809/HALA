...
int main ()
{
	...
	while (1)
	{
		for(i=0; i<4; i++)
      {
        ...
        Delay_us(1000);
        ADC_ConvertedValue[i] = ADC_GetConversionValue(ADC1); // Read ADC Conversion Result
				ADC_ClearFlag(ADC1, ADC_FLAG_EOC);
        switch(i)
        {
          case 0:
            TIM2->CCR1 = map(ADC_ConvertedValue[i]); // Map The ADC_RES To PWM DutyCycle
            break;
          case 1:
            TIM2->CCR2 = map(ADC_ConvertedValue[i]); // Map The ADC_RES To PWM DutyCycle
            break;
          case 2:
            TIM2->CCR3 = map(ADC_ConvertedValue[i]); // Map The ADC_RES To PWM DutyCycle
            break;
          case 3:
            TIM2->CCR4 = map(ADC_ConvertedValue[i]); // Map The ADC_RES To PWM DutyCycle
            break;
        }
      }
			Delay_us(1000);
